...
void message_generation_test(MessageType message_type) {
    ...
    MessageFactory::get_message_data(message_type, data, message);
...
void messag_parsing_test(MessageType message_type) {
    ...
    Parser parser;
    assert(parser.state == ParsingState::FINDING_START_FLAG);
    parsing_result = parser.parse_byte(message[0]);
    assert(parser.state == ParsingState::FINDING_MESSAGE_LABEL);
    parsing_result = parser.parse_byte(message[1]);
    assert(parser.state == ParsingState::PARSING_MESSAGE);
    parsing_result = parser.parse_byte(message[2]);
    assert(parser.state == ParsingState::FINDING_END_FLAG);
