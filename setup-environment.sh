...
#
# INSTALL 'scara' ENVIRONMENT WITH ALL DEPENDENCIES
#
ENVS=$(conda env list | awk '{print $1}' )
if [[ $ENVS = *scara* ]]; then
   echo -e "Anaconda 'scara' environment detected."
   conda activate scara
else
   #
   # INSTALL DEPENDENCIES
   #
   echo -e "Anaconda 'scara' environment not detected.\nInstalling anaconda environment 'scara' together with all the required dependencies..."
   conda update --all -y
   ...
   conda deactivate
   conda activate scara
   echo "Run 'conda activate scara' to enable python virtual environment to run the scripts."
fi;
